<?php
function generarPDFCuentaGastos($id)
{
    ...
    LEFT JOIN 2202clavepedimento cp ON r.ClavePedimento = cp.id2202clave_pedimento
...
// Establecer fuente
    ...
    $stmtPartidas = $con->prepare("SELECT * FROM conta_partidaspolizas WHERE ReferenciaId = :id AND EnKardex != 1 AND CuentaCont !=1 AND Activo = 1");
    ...
    foreach ($partidas as $partida) {
        ...
        $cargo = $partida['Cargo'];
        $obsCuenta = $partida['Observaciones'];
    ...
    ");
        ...
        if ($cuenta) {
            ...
            $pdf->Cell(150, $lineHeight, $nombreCuenta . '(' . $obsCuenta . ')', 0, 0, 'L');
